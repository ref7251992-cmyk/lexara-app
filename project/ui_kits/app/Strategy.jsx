// Strategy — agentic chat surface for negotiation strategy.
const SEED = [
  { who: "user", text: "Acme прислал v3.2. Покажи, что изменилось, и где стоит давить." },
  { who: "agent", thinking: [
...
const Strategy = ({ contract }) => {
  ...
  return (
    <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr", overflow: "hidden", background: "var(--bg)" }}>
        ...
        <div style={{ overflow: "auto", padding: "28px 32px 16px", flex: 1 }}>
          <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div className="t-h5" style={{ color: "var(--fg-1)" }}>Стратегия переговоров</div>
              <Badge tone="brand">Активно · {contract?.title || "Рамочный договор — Acme Corp"}</Badge>
            </div>

            {msgs.map((m, i) => m.who === "user" ? <UserMsg key={i} text={m.text} /> : <AgentMsg key={i} {...m} />)}
            {busy && <ThinkingPulse />}
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid var(--border)", padding: "14px 32px 18px", background: "white" }}>
          <div style={{ maxWidth: 760, margin: "0 auto" }}>
            <div className="glass" style={{
              ...
              boxShadow: "var(--shadow-sm)", backdropFilter: "none",
            }}>
...
const UserMsg = ({ text }) => (
  <div style={{ display: "flex", justifyContent: "flex-end" }}>
    <div style={{
      ...
      font: "400 14px/22px var(--font-sans)", boxShadow: "var(--shadow-sm)",
    }}>{text}</div>
  </div>
);

const AgentMsg = ({ thinking, text, moves }) => (
  <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
    <div style={{ width: 28, height: 28, borderRadius: 999,
      background: "linear-gradient(135deg,#3D3FE2,#7147FF)",
      display: "flex", alignItems: "center", justifyContent: "center", flex: "none" }}>
      <Icon name="sparkles" size={14} style={{ color: "white" }} />
    </div>
    <div style={{ flex: 1, minWidth: 0 }}>
      {thinking && (
        <div className="card" style={{ padding: "10px 12px", marginBottom: 8, background: "var(--n-10)", border: "1px solid var(--divider)", boxShadow: "none" }}>
...
const MoveCard = ({ tone, title, body }) => {
  const tones = {
  ...
  return (
    <div className="card" style={{ padding: 14, display: "flex", gap: 12, alignItems: "flex-start" }}>
      <div style={{ width: 32, height: 32, borderRadius: 8, background: t.bg, color: t.fg,
        display: "flex", alignItems: "center", justifyContent: "center", flex: "none" }}>
...
const ThinkingPulse = () => (
  <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
    <div style={{ width: 28, height: 28, borderRadius: 999, background: "linear-gradient(135deg,#3D3FE2,#7147FF)",
    ...
    <div style={{ display: "flex", gap: 4 }}>
      {[0,1,2].map(i => (
        <span key={i} style={{
          ...
          animation: `lx-pulse 1.2s ${i * 0.15}s infinite var(--ease)`,
        }} />
      ))}
    </div>
