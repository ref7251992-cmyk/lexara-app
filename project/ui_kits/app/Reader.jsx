// Reader — long-form contract document with Lexara annotations.
const CLAUSES = [
  { id: "p1", n: "1", title: "Определения", body: "В настоящем Договоре термины, указанные с заглавной буквы, имеют значения, определённые в Приложении А. Если термин не определён, он толкуется в его обычном коммерческом смысле в соответствии с законодательством штата Делавэр." },
  { id: "p2", n: "2", title: "Услуги", body: "Поставщик оказывает Услуги, описанные в каждом Заказе, заключённом во исполнение настоящего Договора. Каждый Заказ является неотъемлемой частью настоящего Договора и подчиняется его условиям." },
  { id: "p7", n: "7.4", title: "Возмещение убытков", body: "Совокупная ответственность Поставщика по настоящему Договору не может превышать сумму платежей, уплаченных Заказчиком за шесть (6) месяцев, предшествующих требованию. Данное ограничение применяется независимо от формы предъявленного требования.",
  ...
  { id: "p12", n: "12.1", title: "Срок действия и расторжение", body: "Настоящий Договор действует тридцать шесть (36) месяцев с Даты вступления в силу, если не будет расторгнут досрочно в соответствии с настоящим разделом.",
  ...
  { id: "p15", n: "15", title: "Конфиденциальность", body: "Каждая Сторона обязуется сохранять конфиденциальность непубличной информации другой Стороны и не раскрывать такую информацию, кроме как своим аффилированным лицам и консультантам, принявшим на себя эквивалентные обязательства." },
];

const Reader = ({ contract, onPickClause, picked }) => (
  <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 384px", overflow: "hidden" }}>
    <div style={{ overflow: "auto", padding: "32px 48px", background: "var(--bg)" }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        <div style={{ font: "500 12px/16px var(--font-mono)", color: "var(--fg-3)", textTransform: "uppercase", letterSpacing: "0.04em" }}>{contract.v} · посл. ред. сегодня в 14:02</div>
        <h1 className="t-h4" style={{ marginTop: 6, color: "var(--fg-1)" }}>{contract.title}</h1>
        ...
        <div style={{ marginTop: 28, display: "flex", flexDirection: "column", gap: 22 }}>
          {CLAUSES.map(cl => {
            ...
            return (
              <div key={cl.id} onClick={() => cl.flag && onPickClause(cl.id)} style={{
                position: "relative", cursor: cl.flag ? "pointer" : "default",
                paddingLeft: 16,
                borderLeft: cl.flag ? `3px solid ${cl.flag.tone === "danger" ? "var(--danger)" : "var(--warning)"}` : "3px solid transparent",
              }}>
                ...
                <div style={{ font: "400 16px/26px var(--font-sans)", color: "var(--fg-2)", marginTop: 8 }}>
                  {cl.body}
                </div>
                {cl.flag && (
                  <div style={{
    ...
    <Inspector pickedId={picked} onPickClause={onPickClause} />
  </div>
);

const Inspector = ({ pickedId, onPickClause }) => {
  const cl = CLAUSES.find(c => c.id === pickedId) || CLAUSES.find(c => c.flag);
  if (!cl || !cl.flag) return null;
  return (
    <aside style={{
      borderLeft: "1px solid var(--border)", background: "white",
      padding: "24px 22px", overflow: "auto", display: "flex", flexDirection: "column", gap: 16,
    }}>
      <div>
      ...
      <Card inset padding={14}>
        ...
        <div style={{ font: "400 14px/22px var(--font-sans)", color: "var(--fg-1)", marginTop: 6 }}>
          Предложить лимит ответственности <span className="t-mono" style={{ background: "#E5F4E5", color: "#005C00", padding: "1px 4px", borderRadius: 3 }}>24 месяца</span> и добавить исключение по нарушению ИС.
        </div>
      ...
      <div>
        ...
        <div className="t-mono" style={{ font: "400 13px/22px var(--font-mono)", padding: 12, borderRadius: 10, background: "var(--n-10)", border: "1px solid var(--border)" }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span className="t-mono" style={{ font: "500 13px/18px var(--font-mono)", color: "var(--fg-1)" }}>{cl.flag.confidence}%</span>
        </div>
